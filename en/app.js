...
function setLanguage(language) {
  ...
  // Always allow manual language switching via dropdown
...
function normaliseRow(row){
  ...
  return {
    ...
    category:(get("Category_Eng")|| "").trim(),
...
function filterList(q, selectedCats){
  ...
  const filtered = DATA.filter(d => {
    ...
    const matchesC = selectedCats.length === 0 || selectedCats.includes(d.category);
    
    // Debug logging for category matching
    if (selectedCats.length > 0) {
      console.log(`Proposal "${d.title.substring(0, 30)}..." category: "${d.category}" | Selected: [${selectedCats.join(', ')}] | Matches: ${matchesC}`);
    }
    
    return matchesQ && matchesC;
  });
  
  console.log(`🎯 Filter result: ${filtered.length} of ${DATA.length} proposals shown`);
  return filtered;
}
...
async function renderFilters(){
  ...
  const cats = uniques(DATA.map(d => d.category)).filter(Boolean);
  ...
  if (cats.length === 0) {
    menu.innerHTML = '<div class="ms-item" style="color: #666; font-style: italic;">No categories available</div>';
  }
  ...
  if (mobileCatMs) {
    ...
    const mobileItems = ['All categories', ...cats].map(c => {
      return `<div class="ms-item" role="option" data-value="${c}" aria-selected="${c==='All categories' ? 'true' : 'false'}">
        <span class="ms-check">${c==='All categories' ? '•' : ''}</span>
        <span class="ms-label">${c}</span>
      </div>`;
        ...
        <div class="card-body">
          ...
          <div class="meta">
           ...
           <div class="actions">
            ...
            ${d.pdfUrl ? `<a class="download" href="${d.pdfUrl}" target="_blank" rel="noopener">Download the analysis</a>` : ""}
           </div>
        </div>
      </div>
    </article>`;
...
async function init(){
  ...
  // Initialize language - prevent auto-redirect on page load but allow manual switching
  const currentPath = window.location.pathname;
  const savedLanguage = localStorage.getItem('selectedLanguage');
  
  if (currentPath.includes('/en/')) {
    // When in English directory, set English as current language
    CURRENT_LANGUAGE = 'en';
    localStorage.setItem('selectedLanguage', 'en');
    console.log('Set English as current language for /en/ directory');
    
    // Update language selectors to show English
    const languageSelect = document.getElementById('languageSelect');
    const mobileLanguageSelect = document.getElementById('mobileLanguageSelect');
    if (languageSelect) languageSelect.value = 'en';
    if (mobileLanguageSelect) mobileLanguageSelect.value = 'en';
    
    // Load data for English
    loadDataForCurrentLanguage();
  } else {
    // For other directories, use saved language preference or default
...
// ========== Public Proposals Tracker Functions ==========
...
// Tracker search functionality
let trackerSearchTerm = '';
let filteredTrackerData = [];

// Google Sheets CSV URL for budget concepts (specific sheet)
// Note: We need to get the GID of the "Concepts for the Budget" sheet
// For now, using a placeholder - you'll need to get the actual GID
...
function getItemsPerPage() {
  if (window.innerWidth <= 768) {
    return 3; // Mobile and tablet - 3 rows
  }
  return 4; // Desktop
}
...
// Load data from Google Sheets and populate the table with pagination
async function loadProposalsTrackerData() {
  ...
  try {
    ...
    console.error('Error loading proposals tracker data:', error);
...
// Re-render tracker when window is resized to switch between desktop/mobile layouts
let resizeTimeout;
window.addEventListener('resize', function() {
  clearTimeout(resizeTimeout);
  resizeTimeout = setTimeout(() => {
    const modal = document.getElementById('proposalsTrackerModal');
    if (modal && modal.style.display === 'flex' && trackerData && trackerData.length > 0) {
      renderPaginatedData();
    }
  }, 250);
});

// ========== Budget Concepts Functions ==========
    ...
    conceptsContainer.innerHTML = `
      <div class="concepts-placeholder">
        <div class="placeholder-content">
          ...
          <p>To access the "Concepts for the Budget" sheet:</p>
          <ol style="text-align: left; margin: 20px 0;">
            <li>Open your Google Sheet</li>
            <li>Click on the "Concepts for the Budget" tab</li>
            <li>Copy the GID number from the URL (after #gid=)</li>
            <li>Update the BUDGET_CONCEPTS_CSV_URL with the correct GID</li>
          </ol>
          <p><strong>Current URL:</strong> ${BUDGET_CONCEPTS_CSV_URL}</p>
        </div>
      </div>
    `;
  }
