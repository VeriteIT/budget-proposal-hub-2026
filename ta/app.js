...
function normaliseRow(row){
  ...
  return {
    ...
    category:(get("Category_Tam")|| "").trim(),
...
function filterList(q, selectedCats){
  ...
  console.log('🔍 Tamil Filter Debug:', {
    query: query,
  ...
  const filtered = DATA.filter(d => {
    ...
    if (selectedCats.length === 0) {
      ...
      const reverseTranslations = {
        ...
        'நீதி மற்றும் உரிமைகள்': 'Justice and Rights',
        'ஆட்சி': 'Governance',
        'வருவாய் உருவாக்கம்': 'Revenue Generation'
      };
    ...
    if (selectedCats.length > 0) {
      console.log(`Tamil Proposal "${d.title.substring(0, 30)}..." category: "${d.category}" | Selected: [${selectedCats.join(', ')}] | Matches: ${matchesC}`);
    }
    
    return matchesQ && matchesC;
  });
  
  console.log(`🎯 Tamil Filter result: ${filtered.length} of ${DATA.length} proposals shown`);
  return filtered;
}
...
async function renderFilters(){
  ...
  // Translate English categories to Tamil
  const categoryTranslations = {
    'Economic Growth': 'பொருளாதார வளர்ச்சி',
    'Justice and Rights': 'நீதி மற்றும் உரிமைகள்',
    'Governance': 'ஆட்சி',
  ...
  sel.innerHTML = ['அனைத்து பிரிவுகளும்', ...cats].map(c => `<option>${c}</option>`).join('');
  sel.value = 'அனைத்து பிரிவுகளும்';

  // Build custom menu
  const items = ['அனைத்து பிரிவுகளும்', ...cats].map(c => {
    return `<div class="ms-item" role="option" data-value="${c}" aria-selected="${c==='அனைத்து பிரிவுகளும்' ? 'true' : 'false'}">
      <span class="ms-check">${c==='அனைத்து பிரிவுகளும்' ? '•' : ''}</span>
  ...
  if (cats.length === 0) {
    menu.innerHTML = '<div class="ms-item" style="color: #666; font-style: italic;">பிரிவுகள் இல்லை</div>';
  }
  ...
  // Selection model
  let selected = new Set(['அனைத்து பிரிவுகளும்']);
  ...
  const updateUI = () => {
    ...
    // reflect summary
    const shown = selected.has('அனைத்து பிரிவுகளும்') ? 'அனைத்து பிரிவுகளும்'
  ...
  menu.addEventListener('click', (e) => {
    ...
    if (val === 'அனைத்து பிரிவுகளும்') {
      // Selecting "All" clears others
      selected.clear();
      selected.add('அனைத்து பிரிவுகளும்');
    } else {
      ...
      selected.delete('அனைத்து பிரிவுகளும்');
  ...
  window.getSelectedCategories = () => {
    return selected.has('அனைத்து பிரிவுகளும்') ? [] : Array.from(selected);
  };
  ...
  if (mobileCatMs) {
    ...
    const mobileItems = ['அனைத்து பிரிவுகளும்', ...cats].map(c => {
      return `<div class="ms-item" role="option" data-value="${c}" aria-selected="${c==='அனைத்து பிரிவுகளும்' ? 'true' : 'false'}">
        <span class="ms-check">${c==='அனைத்து பிரிவுகளும்' ? '•' : ''}</span>
        <span class="ms-label">${c}</span>
      </div>`;
    ...
    mobileMenu.addEventListener('click', (e) => {
      ...
      if (val === 'அனைத்து பிரிவுகளும்') {
        ...
        selected.delete('அனைத்து பிரிவுகளும்');
        // If none left, fall back to All
        if (selected.size === 0) selected.add('அனைத்து பிரிவுகளும்');
      }
...
function card(d){
    ...
    const voteSection = d.voteFormUrl ? 
      `<div class="vote-section">
        <a href="${d.voteFormUrl}" target="_blank" rel="noopener" class="vote-pill" aria-label="Vote for this proposal" onclick="event.stopPropagation();">
          <span>இப்போது வாக்களியுங்கள்</span>
    ...
    if (!categoryColor) {
      const categoryMap = {
        ...
        'பொருளாதார வளர்ச்சி': 'blue',
        'நீதி மற்றும் உரிமைகள்': 'yellow',
        'ஆட்சி': 'red',
        'வருவாய் உருவாக்கம்': 'yellow'
      };
        ...
        <div class="card-body">
          ...
          <div class="meta">
           ...
           <div class="actions">
            ...
            ${d.pdfUrl ? `<a class="download" href="${d.pdfUrl}" target="_blank" rel="noopener">பகுப்பாய்வைப் பதிவிறக்கவும்</a>` : ""}
           </div>
        </div>
      </div>
    </article>`;
    ...
    proposalsContainer.innerHTML = `
      <div class="concepts-placeholder">
        <div class="placeholder-content">
          ...
          <h3>மற்ற வரவு செலவுத் திட்ட முன்மொழிவுகள்</h3>
